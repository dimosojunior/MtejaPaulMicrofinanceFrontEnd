...
import { View, SafeAreaView, ImageBackground, TextInput, Alert,Platform, Image, StyleSheet, ActivityIndicator, Text, Dimensions, ScrollView, TouchableOpacity } from 'react-native';
...
async function registerForPushNotificationsAsync() {
  ...
  if (Device.isDevice) {
    ...
    if (finalStatus !== 'granted') {
      handleRegistrationError('Haujaruhusu kupata ujumbe kutoka ndani ya kifaa chako');
      return;
    }
 
 const projectId = '8c8e8cdf-bdf4-4df2-8993-1f7614db109d';
 if(!projectId) {
  handleRegistrationError('Imeshindikana kupata ID ya kifaa');
 }
 try {
  ...
  handleRegistrationError('Kifaa unachotumia kimeshindwa kupokea ujumbe');
}
...
// State variable to store the RoomClasses data
 ...
 const [selectedRole, setSelectedRole] = useState(null);
  ...
  useEffect(() => {
    AsyncStorage.getItem("userToken").then(token2 => {
      setUserToken(token2)
    })
  }, [userData]);
...
<View style={{ 
  ...
  <View style={[globalStyles.input,
    ...
    <FontAwesome style={globalStyles.InputIcon} 
    name='image' 
    color= "white"
    />
    <TouchableOpacity
    ...
    >
      <Text style={{ 
        color: 'white',
        marginLeft:15, 
      }}>Bonyeza kuchagua picha yako</Text>
    </TouchableOpacity>
  </View>
