...
const FutaRipoti = ({navigation, route}) => {
...
const handleDeletePost = async () => {
    ...
    try {
      await axios.delete(EndPoint + `/DeleteRipotiView/${postId}/delete/`, {
        headers: {
          Authorization: `Token ${userToken}`,
        },
      });
      showAlertFunction('Umefanikiwa kufuta ripoti');
      navigation.replace('Ripoti Ya Siku');  // Navigate back to the previous screen
    } catch (error) {
      showAlertFunction('Imeshindikana kufuta ripoti');
      //console.log(error);
    }
  };
...
{MapatoYaJumla > 0 ? (
 <Text style={globalStyles.TaarifaBinafsiSimuYaMtejaMkopoText}>
     Mapato Ya Jumla: {formatToThreeDigits(MapatoYaJumla)}    
      </Text>
      ):(
    <Text style={globalStyles.TaarifaBinafsiSimuYaMtejaMkopoText}>
     Mapato Ya Jumla: 0   
    ...
    {MatumiziYaJumla> 0 ? (
     ...
     Matumizi Ya Jumla: {formatToThreeDigits(MatumiziYaJumla)}    
      </Text>
      ):(
 <Text style={globalStyles.TaarifaBinafsiSimuYaMtejaDeniText}>
     Matumizi Ya Jumla: 0    
