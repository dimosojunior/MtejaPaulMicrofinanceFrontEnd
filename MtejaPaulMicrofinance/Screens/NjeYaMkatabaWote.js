...
const NjeYaMkatabaWote = ({ navigation }) => {
...
const getItems = (token) => {
  if (endReached) {
   ...
   const url = EndPoint + `/GetWatejaNjeYaMkatabaWoteView/?page=${current_page}&page_size=500`
    // console.log(url);
    fetch(url, {
      method: 'GET',
      headers: {
...
const handleRefresh = async () => {
  ...
  try {
    ...
    if (token) {
      ...
      const url = EndPoint + `/GetWatejaNjeYaMkatabaWoteView/?page=1&page_size=500`;
...
export default NjeYaMkatabaWote;
