...
const HawajarejeshaJana = ({ navigation }) => {
...
const getItems = (token) => {
  if (endReached) {
   ...
   const url = EndPoint + `/GetHawajarejeshaJanaView/?page=${current_page}&page_size=500`
    // console.log(url);
    fetch(url, {
      method: 'GET',
      headers: {
...
const handleRefresh = async () => {
  ...
  try {
    ...
    if (token) {
      ...
      const url = EndPoint + `/GetHawajarejeshaJanaView/?page=1&page_size=500`;
 ...
 {!isPending ? (
        ...
        <View style={globalStyles.container}>
          ...
          <Text
            ...
            >
              Hawajarejesha tarehe  {formatDate(startDate)}
            </Text>
            )}
...
export default HawajarejeshaJana;
