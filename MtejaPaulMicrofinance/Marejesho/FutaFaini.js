...
const FutaFaini = ({navigation, route}) => {
...
const handleDeletePost = async () => {
    ...
    try {
      await axios.delete(EndPoint + `/DeleteFainiView/${postId}/delete/`, {
        headers: {
          Authorization: `Token ${userToken}`,
        },
      });
      showAlertFunction('Umefanikiwa kufuta faini');
      navigation.replace('Pokea Rejesho HomeScreen');  // Navigate back to the previous screen
    } catch (error) {
      showAlertFunction('Imeshindikana kufuta faini');
      //console.log(error);
    }
  };
...
<View style={{
  ...
  }}>Futa faini ya  {JinaKamiliLaMteja}</Text>
</View>
          ...
          >
            ...
            // padding:13,
             ...
             paddingVertical:10,
              ...
              Futa => {formatToThreeDigits(FainiIliyoPokelewaLeo)}
            </Text>
          </TouchableOpacity>
